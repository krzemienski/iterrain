...
- (void)setupDefaultAttributes 
{
    ...
    number.font = [UIFont fontWithName:@"Helvetica-Bold" size:25];
...
- (id)initWithCoder:(NSCoder *)aDecoder 
{
    if ((self = [super initWithCoder:aDecoder])) 
    {
        ...
        self.imageView.image = [aDecoder decodeObjectForKey:@"image"];
        self.commentString = [aDecoder decodeObjectForKey:@"commentString"];
        self.number.text = [aDecoder decodeObjectForKey:@"number.text"];
        self.number.frame = [aDecoder decodeCGRectForKey:@"number.frame"];
        self.wrapper = [aDecoder decodeObjectForKey:@"wrapper"];
        self.wrapper.frame = [aDecoder decodeCGRectForKey:@"wrapper.frame"];
    }
    return self;
}
...
-(void)encodeWithCoder:(NSCoder *)encoder
{
    ...
    [encoder encodeObject:self.imageView.image forKey:@"image"];
    [encoder encodeObject:self.imageView forKey:@"imageView"];
    [encoder encodeObject:self.commentString forKey:@"commentString"];
    [encoder encodeObject:self.number.text forKey:@"number.text"];
    [encoder encodeCGRect:self.number.frame forKey:@"number.frame"];
    [encoder encodeObject:self.wrapper forKey:@"wrapper"];
    [encoder encodeCGRect:self.wrapper.frame forKey:@"wrapper.frame"];
}
...
- (void)resizeUsingTouchLocation:(CGPoint)touchPoint 
{
    ...
    //Check for the views and if they are a circle or a square then make sure to tell it to scale to the new frame.
    for (UIView * to_be_tested in self.subviews) 
    {
        if ([to_be_tested isKindOfClass:[Circle class]] || [to_be_tested isKindOfClass:[Rectangle class]]) 
        {
            [to_be_tested setNeedsDisplay];
            
        }
    }
    for (UIView * to_be_tested in self.contentView.subviews) 
    {
        //NSLog(@"%d",self.subviews.count);
        if ([to_be_tested isKindOfClass:[UITextView class]]) 
        {
            
            to_be_tested.frame = CGRectMake(0, 20, self.frame.size.width-20, self.frame.size.height-40);
...
{     
        ...
        for (UIView * to_be_tested in self.subviews) 
        {
            if ([to_be_tested isKindOfClass:[UITextView class]]) 
            {
                in_view = (UITextView*)to_be_tested;   
            }
        }
        CGPoint pointInB = [in_view convertPoint:point fromView:self];
        
        if ([in_view pointInside:pointInB withEvent:event])
            return in_view;
        else 
        {
         return [super hitTest:point withEvent:event];     
...
-(void)select_text_field
{
    for (UIView * to_be_tested in self.contentView.subviews) 
    {
        ...
        if ([to_be_tested isKindOfClass:[UITextView class]]) 
        {
            ...
            [to_be_tested becomeFirstResponder];
